...
items[4001] = {
    ...
    BattleUse = function(user, target, battle_item)
        local battle_instance = ModeManager:GetTop();
        battle_item:LogEscapeSmokeAttempt(user);

        -- If it's a boss battle, this can't work at all
        if (battle_instance:IsBossBattle() == true) then
            battle_item:LogEscapeSmokeResult(user, false, "boss");
            return true;
        end
        ...
        if (math.random(0, 100) > evade_diff) then
            local target_actor = target:GetActor();
            battle_item:LogEscapeSmokeResult(user, false, "chance_failed");
            return true;
        end

        battle_item:LogEscapeSmokeResult(user, true, "success");
